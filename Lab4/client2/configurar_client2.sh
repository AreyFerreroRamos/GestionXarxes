...
ifdown eth0
ifup eth0
# S'executa la comanda 'ip -c a' o 'ip -f inet address show eth0' (més específica) per comprovar si s'ha afegit una adreça IP dinàmica a la interficie eth0. Si hi apareix, és que s'ha afegit.
...
# S'executa la comanda 'ping router' per comprovar que es pot establir una connexió amb el router utilitzant el nom asignat.
# S'executa la comanda 'ping server' per comprovar que es pot establir una connexió amb el server utilitzant el nom asignat.
# S'executa la comanda 'ping intern' per comprovar que es pot establir una connexió amb l'intern utilitzant el nom asignat.
...
service ssh restart
# S'executa la comanda 'ssh root@router' per comprovar que es poden crear arxius al router des del client2.
# S'executa la comanda 'ssh -rvp client2 root@router:/home/milax/' per passar arxius del client2 al router.
